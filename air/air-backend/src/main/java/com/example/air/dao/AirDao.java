...
import com.example.air.pojo.UserPojo;
...
public class AirDao {
    ...
    public boolean insertAirdata(int peroid,String account,String num) {
        final String sql = " insert into air(peroid,account,num)values(?,?,?)";
        jdbcTemplate.update(sql,peroid,account,num);
        return true;
    }

    public int getLastPeroid(){
        final String sql = "select count(*) from lotterylist";
        return jdbcTemplate.queryForObject(sql,int.class);
    }

    public String getUserData(int peroid,String account) {
        String sql = "select num  from lotterylist where  peroid  = ? and account =?";
        List userPojoList = new ArrayList();
        String num = jdbcTemplate.queryForObject(sql, new Object[]{peroid,account},String.class);
        return num;
    }


    public List getUserData(int peroid) {
        String sql = "select peroid,account,num  from air where  peroid  = ?";
        List list = new ArrayList();
        SqlRowSet sqlRowser = jdbcTemplate.queryForRowSet (sql,new Object[]{peroid});
        while (sqlRowser.next ()) {
            UserPojo userPojo = new UserPojo();
            userPojo.setPeroid(peroid);
            userPojo.setAccount(sqlRowser.getString ("account"));
            userPojo.setNum(sqlRowser.getString("num"));
            list.add (userPojo);
        }
        return list;
    }
