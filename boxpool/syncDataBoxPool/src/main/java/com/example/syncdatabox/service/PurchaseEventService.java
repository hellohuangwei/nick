...
public class PurchaseEventService {
    ...
    public static final Event purchase_EVENT = new Event("purchaseEvent",
...
//        web3j = Web3Intence.getInstance("https://bsc-daaseed1.defibit.io");
        ...
        String number_start = blockNumberDao.showContractInfo(0).getStart();
        String number_end = blockNumberDao.showContractInfo(0).getEnd();
        ...
        if (logsList != null){
            try {
                for (int i =0;i<logsList.size();i++){
                    ...
                    purchasePojo.setAccount(String.valueOf(delogResults.get(0).getValue()));
                    purchasePojo.setPeroid(Integer.valueOf(delogResults.get(1).getValue().toString()));
                    purchasePojo.setPrice(String.valueOf(delogResults.get(2).getValue()));
                    purchasePojo.setRandomNum(Integer.valueOf(delogResults.get(3).getValue().toString()));
                    purchasePojo.setTimestamp(String.valueOf(delogResults.get(4).getValue()));
                    purchasePojo.setTxHash(((Log) logsList.get(i)).getTransactionHash());
                    purchasePojo.setAward(0);
                    purchasePojo.setApply(0);
                    boxService.insertUserData(purchasePojo);
                }
            }catch (Exception e){
                e.printStackTrace();
